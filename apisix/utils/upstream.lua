...
local function is_private_ip(ip)
    ...
    if a == "172" and tonumber(b) >= 16 and tonumber(b) <= 31 then
        return true
    end
...
_M.is_private_ip = is_private_ip
        ...
        if not ipmatcher.parse_ipv4(host) and
            ...
            if ip then
                if is_private_ip(ip) then
                    core.log.error("dns resolver domain: ", host, " ip: ", ip, " . This is private ip address")
                else
