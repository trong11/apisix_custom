...
function _M.new(plugin_name, limit)
    assert(limit > 0)

    local self = {
        limit_count = limit_local_new(plugin_name, limit, 31536000),
        dict = ngx.shared["plugin-limit-count-year-reset-header"]
    }
...
function _M.incoming(self, key, commit, conf)
    ...
    if remaining == conf.count - 1 then
        reset = set_endtime(self, key, 31536000)
    else
