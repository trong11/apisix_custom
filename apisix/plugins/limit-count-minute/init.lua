...
local plugin_name = "limit-count-minute"
local limit_redis_cluster_new
local limit_redis_new
local limit_local_new
do
    local local_src = "apisix.plugins.limit-count-minute.limit-count-minute-local"
    limit_local_new = require(local_src).new

    local redis_src = "apisix.plugins.limit-count-minute.limit-count-minute-redis"
    limit_redis_new = require(redis_src).new

    local cluster_src = "apisix.plugins.limit-count-minute.limit-count-minute-redis-cluster"
...
local function create_limit_obj(conf)
    ...
    if conf.policy == "redis" then
        return limit_redis_new("plugin-" .. plugin_name, conf.count, conf)
    end

    if conf.policy == "redis-cluster" then
        return limit_redis_cluster_new("plugin-" .. plugin_name, conf.count, conf)
    end
...
function _M.rate_limit(conf, ctx)
    ...
    if not lim then
        ...
        if conf.allow_degradatioan then
            return
        end
        return 500
    end
